...
public class reducer_helper {
    ...
    public static void main(String[] args) throws IOException, InterruptedException {
        ...
        // one could omit this since the reducer_helper is called from reducer.sh (so if
        // that is correctly coded this error should not happen)
        if (args.length != 2) {
        ...
        // Precompute expected output(s), so we don't have to compute the error for the
        // original_test again and again and again...
        Path expected1 = null;
        Path expected2 = null;
        ...
        File[] files = sql_queries.toFile()
                .listFiles((dir, name) -> name.startsWith("query_") && name.endsWith(".sql"));
        ...
        for (int i = 1; i <= order; i++) {
            ...
            // Build pre_next_sql: we attach to pre_next_sql the reduced sql from the query
            // before
            ...
            // If completely excluding the next_sql query actually gives us the same exit
            // code as including it (resp. there still exists a failure), we can just reduce
            // to the empty string (we do not need it to preserve the error)
            // we can do this to simply speed up the reduce process
            if (test_for_fail("", test, oracleTempPath.toString(), originalTestTempPath.toString(), pre_next_sql,
                    post_next_sql, expected1, expected2) == 0) {
                reduced_sql = "";
            } else {
                reduced_sql = reduce(next_sql, 2, test, oracleTempPath.toString(), originalTestTempPath.toString(),
                        pre_next_sql, post_next_sql, expected1, expected2);
            }
            System.out.println("Reduced query: " + reduced_sql);

            // Build pre_next_sql: we attach to pre_next_sql the reduced sql from the query
            // before
            pre_next_sql = pre_next_sql + '\n' + reduced_sql;

            // we set post_next_sql again to the empty string
            post_next_sql = "";
        }

        // since pre_next_sql actually also contains all our reduced sqls we have our
        // reduced.sql
    ...
    private static String reduce(String curr_sql_line, int n, String testScript, String oracle_path,
            String original_test_path, String pre_next_sql, String post_next_sql, Path expected1, Path expected2)
            throws IOException, InterruptedException {
        if (curr_sql_line.length() == 0) {
            return curr_sql_line;
        }

        // now if we reached a granularity of n = to the length of the current reduced
        // sql query, we know there is nothing we can reduce further.
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (test_for_fail(parts.get(i), testScript, oracle_path, original_test_path, pre_next_sql, post_next_sql,
                    expected1, expected2) == 0) {
                // call reduce with said delta and n = 2
                return reduce(parts.get(i), 2, testScript, oracle_path, original_test_path, pre_next_sql, post_next_sql,
                        expected1, expected2);
            }
            // if there is a failure when taking the complement of delta_i as reduced sql
            // query then....
            if (test_for_fail(comps.get(i), testScript, oracle_path, original_test_path, pre_next_sql, post_next_sql,
                    expected1, expected2) == 0) {
                // call reduce with said complement of delta and n = n -1
                return reduce(comps.get(i), n - 1, testScript, oracle_path, original_test_path, pre_next_sql,
                        post_next_sql, expected1, expected2);
            }
        }
        ...
        // }
        // else{
        // if there is still "reduction potential" we just adapt the value n, we
        // multiply it by a factor of 2 (as described in the algo as well)
        return reduce(curr_sql_line, n * 2, testScript, oracle_path, original_test_path, pre_next_sql, post_next_sql,
                expected1, expected2);
        // }
    ...
    // private static int test_for_fail(String sql_query, String testScript, String
    // oracle, String original_test_sql) throws IOException, InterruptedException {
    private static int test_for_fail(String sql_query, String testScript, String oracle_path, String original_test_path,
            String pre_next_sql, String post_next_sql, Path expected1, Path expected2)
            throws IOException, InterruptedException {
        ...
        // oracle_path, original_test_path);
        ProcessBuilder pb = new ProcessBuilder(testScript, tmpQuery.toString(), oracle_path, original_test_path,
                expected1 != null ? expected1.toString() : "NULL", expected2 != null ? expected2.toString() : "NULL");
